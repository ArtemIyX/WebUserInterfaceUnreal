...
namespace
{
	...
	TAutoConsoleVariable<int32> CVarCefWsShutdownDrainMs(TEXT("cefws.shutdown_drain_ms"), 100, TEXT("Drain window before stopping websocket threads (milliseconds)."));
	...
	TAutoConsoleVariable<float> CVarCefWsHeartbeatIntervalSec(TEXT("cefws.heartbeat_interval_sec"), 0.0f, TEXT("Heartbeat send interval in seconds. <=0 disables heartbeat."));
...
namespace CefWebSocketCVars
{
	...
	int32 GetShutdownDrainMs() { return CVarCefWsShutdownDrainMs.GetValueOnAnyThread(); }
